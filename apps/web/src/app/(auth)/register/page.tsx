...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await api.post<{ data: { accessToken: string } }>("/auth/register", form);
  ...
  const FIELDS: { key: keyof typeof form; label: string; type: string; placeholder: string }[] = [
    { key: "displayName", label: "Full name",  type: "text",     placeholder: "Jane Smith" },
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-brand-600 flex-col justify-between p-10">
        ...
        <div>
          ...
          <p className="text-brand-200 text-sm leading-relaxed">
            Join teams who save hours every week by letting CoTask handle the grunt work — from transcript to task in seconds.
          </p>

          <div className="mt-8 space-y-3">
            {["Free 14-day trial", "No credit card required", "Setup in under 2 minutes"].map((f) => (
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-50 px-6">
        <div className="w-full max-w-sm animate-fade-in">
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500">Start turning meetings into tasks</p>
          </div>
